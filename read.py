...
def get_bounding_rectangles(i, j, full_text, coordinates):
    ...
    selected_coords = [coord for k, coord in enumerate(coordinates[i:j+1]) if coord is not None]
    ...
    for coord in selected_coords[1:]:
        if (coord['top'] == current_rect['top'] and
            coord['height'] == current_rect['height'] and
            coord['left'] == current_rect['left'] + current_rect['width']):
